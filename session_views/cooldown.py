...
def render(session):
    ...
    if st.session_state.exercise_index is None:
        if first_incomplete_index is None:
            st.info("Cooldown marked as completed, but you can adjust below.")
    ...
    # ✅ Cooldown header
    warmup_header = current_ex.get("notes", "").strip() or "General Cooldown"
    st.subheader(f"Cooldown Type: {warmup_header}")
    ...
    if st.session_state.running:
        ...
        color = "#0f0" if st.session_state.phase == "exercise" else "#0f0"
